...
def main():
    ...
    print(f"Horizons de prévision: {config.get("data", {}).get("lead_times_hours", [])} heures")
    ...
    print(f"\n🎨 Génération des cartes pour {len(config.get("data", {}).get("lead_times_hours", []))} horizons...")
    
    horizons_found = {h: False for h in config.get("data", {}).get("lead_times_hours", [])}
    ...
    with torch.no_grad():
        for i, batch in enumerate(test_dataloader):
            ...
            for j in range(len(lead_times)):
                ...
                if horizon in config.get("data", {}).get("lead_times_hours", []) and not horizons_found[horizon]:
